...
router.post("/match", async (req, res) => {
	try {
		...
		// const user = await match.findOne({ name: req.body.name });
		// if (user)
		// 	return res
		// 		.status(409)
		// 		.send({ message: "Match with given name already Exist!" });
...
router.get("/allmatch/:id?", function (req, res, next) {
	// single user
	match.find({ _id: req.params.id }).exec(function (error, results) {
	  if (error) {
		return next(error);
	  }
	  // Respond with valid data
